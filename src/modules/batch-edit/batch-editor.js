...
class BatchEditor extends EventEmitter{
            ...
            })
    {
        ...
        this._requests = payloads
            .map(payload => new HttpRequest({
                url: Object.entries(payload.pathParameters).reduce((pathTemplate, [key, value]) => {
                    return pathTemplate.replace(`\${${key}}`,value)
                },this.endpointPath),
                verb: this._endpoint.verb,
     ...
     */
    get waitingCount(){
        return this._requests.filter(req => req.started === false).length;
    }
    ...
    nextRequests() {
        ...
        if(this.allFinished) {
            this.emit('finished', this._requests.map(req => req.result));
            return;
        }
